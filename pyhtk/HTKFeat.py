...
class MFCC_HTK:
    ...
    def load_filter(self, filename: Path):
        ...
        self.filter_num = int(np.max(data[:, 1]))
    ...
    def create_filter(self, num: int):
        ...
        for f in range(self.filter_num):
            ...
            self.filter_mat[point_c[f] : point_c[f + 1] + 1, f] = np.linspace(
                0, 1, d1 + 1
            )
            self.filter_mat[point_c[f + 1] : point_c[f + 2] + 1, f] = np.linspace(
                1, 0, d2 + 1
            )
    ...
    def __init__(
        ...
        feat_melspec: bool = False,
        feat_mfcc: bool = True,
        feat_energy: bool = True,
        ceps_energy: bool = True,
        enormalise: bool = False,
        sil_floor: float = 50.0,
        escale: float = 1.0,
        cmn: bool = False,
    ):
        ...
        self.feat_melspec = feat_melspec
        self.feat_mfcc = feat_mfcc
        self.feat_energy = feat_energy
        self.ceps_energy = ceps_energy
        self.enormalise = enormalise
        self.sil_floor = sil_floor
        self.escale = escale
        self.cmn = cmn
        ...
        for m in range(self.mfcc_num):
            self.dct_base[:, m] = np.cos(
                (m + 1) * np.pi / self.filter_num * (np.arange(self.filter_num) + 0.5)
            )

        self.lifter = 1 + (self.lifter_num / 2) * np.sin(
            np.pi * (1 + np.arange(self.mfcc_num)) / self.lifter_num
        )
        self.mfnorm = np.sqrt(2.0 / self.filter_num)

    @staticmethod
    def load_raw_signal(filename: Path) -> np.ndarray:
    ...
    def get_feats(self, signal: np.ndarray) -> np.ndarray:
        ...
        if self.cmn:
            if self.ceps_energy:
                ret[:, 0 : self.mfcc_num + 1] -= np.mean(
                    ret[:, 0 : self.mfcc_num + 1], axis=0
                )
            else:
    ...
    @staticmethod
    def get_delta(feat: np.ndarray, deltawin: int = 2) -> np.ndarray:
